...
class Agent(object):
    ...
    def act(self, observation, reward, done):
        self.grayscale = np.dot(observation[:, :, :3], [0.299, 0.587, 0.114])
        ...
        if (self.checkCoilyNear(upx, upy) or self.checkCoilyNear(downx,
                                                                 downy) or
                self.checkCoilyNear(leftx, lefty) or self.checkCoilyNear(
                    rightx, righty)):
            return 0
    ...
    def chooseMovement(self, near, far, action):
        ...
        try:
            ...
            if ((self.grayscale[tlnear[0]:brnear[0], tlnear[1]:brnear[1]] == self.coilyColor).any() or
                    self.isEmpty(observation, tlnear, brnear) or
                    (self.grayscale[tlfar[0]:brfar[0], tlfar[1]:brfar[1]] == self.coilyColor).any()):
            ...
            if action == 3 and brnear[1] > 140 and brnear[0] < 210:
                ...
                if ((self.grayscale[tmptl[0]:tmpbr[0], tmptl[1]:tmpbr[1]] == self.coilyColor).any() or
                        (self.grayscale[tmptl2[0]:tmpbr2[0], tmptl2[1]:tmpbr2[1]] == self.coilyColor).any()):
